...
export const LobbyScene = {
    ...
    async initializeLobbyScene() {
        ...
        // Engine with fixed settings to prevent blurriness
        this.engine = new BABYLON.Engine(this.canvas, true, {
            preserveDrawingBuffer: true,
            stencil: true,
            antialias: true,
            adaptToDeviceRatio: false, // Disable to prevent blur during movement
        ...
        // Try to load editor scene first, fallback to procedural
        const sceneConfig = this.getSceneConfig();
        if (sceneConfig.editor_scene) {
            try {
                await this.loadEditorScene(sceneConfig.editor_scene);
            } catch (error) {
                console.warn('Failed to load editor scene, using fallback:', error);
        ...
        // Setup camera controls first
        this.setupCameraControls();

        // Start render loop
        this.startRenderLoop();

        console.log('Lobby scene initialized successfully');

        // Resize handler with proper canvas sizing
        this.resizeHandler = () => {
    ...
    async setupPhysics() {
        try {
            // Use Cannon.js as fallback since Havok has URL issues
            ...
            console.warn('Havok physics failed, using Cannon.js fallback:', error);
            // Fallback to Cannon.js or disable physics
            try {
                // Simple gravity without physics engine for now
                this.scene.gravity = new BABYLON.Vector3(0, -9.81, 0);
                console.log('Basic gravity enabled');
            } catch (fallbackError) {
                console.warn('Physics completely disabled:', fallbackError);
            }
        }
    },
    ...
    async loadEditorScene(scenePath) {
        ...
        try {
            const result = await BABYLON.SceneLoader.ImportMeshAsync(
                "", // Load all meshes
                "/assets/js/babylon/editor/scenes/scene",
                "scene.babylon",
                this.scene
            );

            console.log('Editor scene loaded:', result);
